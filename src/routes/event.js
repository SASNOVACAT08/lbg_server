...
router.post(
  "/",
  async (
    { body: { name, timestamp, isValid, isVisible, gameId }, user: { id } },
    res
  ) => {
    if (!name || !timestamp || !gameId) {
...
router.put(
  "/:id",
  async (
    { params: { id }, body: { name, timestamp, isValid, isVisible } },
    res
  ) => {
    if (!name || !timestamp) {
    ...
    if (typeof timestamp !== "string") {
      return res.send({
        type: "error",
        msg: "This date is not in a string format",
      });
    }
    if (typeof isValid !== "boolean") {
    ...
    if (!event[0]) {
      return res.send({
        type: "error",
        msg: "Event cannot be updated, not available yet",
      });
    }
